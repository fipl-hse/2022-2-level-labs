...
Поэтому IDF для этого слово можно рассчитать по формуле выше, подставив 0 вместо $|\{d_i \in D | t \in d_i\}|$ и 47 вместо $|D|$ (именно столько текстов содержится в рассматриваемой нами коллекции сказок).
...
В этом задании вам нужно написать функцию, которая рассчитывает ожидаемую встречаемость слова. Эта та встречаемость, 
...
Ниже представлены словарь с критическими значениями для разных уровней значимости: 
...
Функция должна вернуть словарь, в который помещены только все статистически значимые слова.
Если на вход подаются некорректные значения, возвращается `None`. 
