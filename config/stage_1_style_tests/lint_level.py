...
def is_passed(lint_output: str, target_lint_level: int) -> int:
    lint_score = 0
    if res := re.search(r'Your code has been rated at \d+\.\d+', lint_output):
        lint_level = res.group(0)
        if num := re.search(r'\d+', lint_level):
            lint_score = int(num.group(0))
...
if __name__ == '__main__':
    ...
    score = int(float(args.target_score))
