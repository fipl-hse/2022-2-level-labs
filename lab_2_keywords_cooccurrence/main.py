    ...
    for phrase in phrases:
        ...
        for word in words:
            if word not in stop_words:
                ...
                candidate_keyword_phrases.append(remove_stop_words)
                remove_stop_words = []
        candidate_keyword_phrases.append(remove_stop_words)
    return [tuple(candidate_phrase) for candidate_phrase in candidate_keyword_phrases if candidate_phrase]
...
def get_top_n(keyword_phrases_with_scores: Mapping[KeyPhrase, float],
    ...
    if not check_dict(keyword_phrases_with_scores, tuple, float, False) or not isinstance(top_n, int) \
        ...
        return None
    top_words_list = []
    sorted_keys = sorted(keyword_phrases_with_scores.keys(), reverse=True,
                         key=lambda word: keyword_phrases_with_scores[word])
    for phrase in sorted_keys:
        if len(phrase) <= max_length:
            top_words_list.append(' '.join(phrase))
...
def calculate_cumulative_score_for_candidates_with_stop_words(candidate_keyword_phrases: KeyPhrases,
    ...
    for phrase in candidate_keyword_phrases:
        candidates_cumulative_score[phrase] = 0.0
        for word in phrase:
            if word not in stop_words:
