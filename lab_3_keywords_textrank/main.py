...
class AdjacencyMatrixGraph:
    ...
    def add_edge(self, vertex1: int, vertex2: int) -> int:
        ...
        for line in self._matrix:
            for _ in self._vertices_list:
                if len(line) < len(self._vertices_list):
                    line.append(0)


        self._matrix[self._vertices_list.index(vertex1)][self._vertices_list.index(vertex2)] = 1
        self._matrix[self._vertices_list.index(vertex2)][self._vertices_list.index(vertex1)] = 1
        return 0
    ...
    def is_incidental(self, vertex1: int, vertex2: int) -> int:
        ...
        """
        if vertex1 not in self._vertices_list or vertex2 not in self._vertices_list:
            return -1
        return self._matrix[self._vertices_list.index(vertex1)][self._vertices_list.index(vertex2)]
    ...
    def calculate_inout_score(self, vertex: int) -> int:
        ...
        if vertex not in self._vertices_list:
            return -1
        return sum(self._matrix[self._vertices_list.index(vertex)])
    ...
    def fill_from_tokens(self, tokens: tuple[int, ...], window_length: int) -> None:
        ...
        ----------
            ...
            window_length: int
                ...
                if they appear in the same window of this length
        """
        ext_pairs = extract_pairs(tokens, window_length)
...
class VanillaTextRank:
    ...
    def update_vertex_score(self, vertex: int, incidental_vertices: list[int], scores: dict[int, float]) -> None:
        ...
        for el in incidental_vertices:
            summ += (1 / self._graph.calculate_inout_score(el)) * scores.get(el, 0)
        self._scores[vertex] = (1 + self._damping_factor) + summ * self._damping_factor
    ...
    def get_top_keywords(self, n_keywords: int) -> tuple[int, ...]:
        ...
        """
        keys_sorted = sorted(self._scores)
        return tuple(sorted(keys_sorted, key=lambda x: - self._scores[x])[:n_keywords])
