...
class TextPreprocessor:
    ...
    def preprocess_text(self, text: str) -> tuple[str, ...]:
        ...
        """
        new_text = self._clean_and_tokenize(text)
        return self._remove_stop_words(new_text)
...
class TextEncoder:
    ...
    def _learn_indices(self, tokens: tuple[str, ...]) -> None:
        ...
        for i, token in enumerate(tokens):
            self._word2id[token] = 1000 + i
            self._id2word[1000 + i] = token
    ...
    for i in range(len(tokens) - window_length):
        ...
        for token1 in window:
            for token2 in window:
                if token1 != token2 and not (token1, token2) not in pairs and (token2, token1) not in pairs:
