...
class TextPreprocessor:
    ...
    def _clean_and_tokenize(self, text: str) -> tuple[str, ...]:
        ...
        return tuple(token for token in splited_text)
    ...
    def _remove_stop_words(self, tokens: tuple[str, ...]) -> tuple[str, ...]:
        ...
        """
        return tuple(i for i in tokens if i not in self._stop_words)
...
class TextEncoder:
    ...
    def encode(self, tokens: tuple[str, ...]) -> Optional[tuple[int, ...]]:
        ...
        numbers_tokens = [self._word2id[token] for token in tokens if token in self._word2id]
        return tuple(numbers_tokens)
...
class AdjacencyMatrixGraph:
    ...
    def add_edge(self, vertex1: int, vertex2: int) -> int:
        ...
        self._matrix[vert1_index][vert2_index] = 1
        self._matrix[vert2_index][vert1_index] = 1
        return 0
    ...
    def is_incidental(self, vertex1: int, vertex2: int) -> int:
        ...
        vert1_index = self._vertices.index(vertex1)
        vert2_index = self._vertices.index(vertex2)
        return self._matrix[vert1_index][vert2_index]
...
class PositionBiasedTextRank(VanillaTextRank):
    ...
    def update_vertex_score(self, vertex: int, incidental_vertices: list[int], scores: dict[int, float]) -> None:
        ...
        for vert in incidental_vertices:
            ...
            sum_list.append(1 / inout_score * self._scores[vert])
        summa = sum(sum_list)
        new_vertex_score = (1 - self._damping_factor) * self._position_weights[vertex] + self._damping_factor * summa
        self._scores[vertex] = new_vertex_score
