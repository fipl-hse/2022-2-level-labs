...
class TextPreprocessor:
    ...
    def preprocess_text(self, text: str) -> tuple[str, ...]:
        ...
        no_stops = self._remove_stop_words(clean_text)
        return no_stops
...
class TextEncoder:
    ...
    def _learn_indices(self, tokens: tuple[str, ...]) -> None:
        ...
        no_same_tokens = []
        for i in tokens:
            if i not in no_same_tokens:
                no_same_tokens.append(i)
        for number, token in enumerate(no_same_tokens, 1000):
...
class AdjacencyMatrixGraph:
    ...
    def calculate_inout_score(self, vertex: int) -> int:
        ...
        if vertex not in self._vertexes:
            return -1
        line = self._matrix[self._vertexes.index(vertex)]
        return sum(line)
    ...
    def fill_positions(self, tokens: tuple[int, ...]) -> None:
        ...
        for number, token in enumerate(tokens):
            if token not in self._positions:
                ...
                self._positions[token].append(number+1)
    ...
    def calculate_position_weights(self) -> None:
        ...
        for token in self._positions:
            weights[token] = sum(1 / number for number in self._positions[token])
        for token in self._positions:
...
class EdgeListGraph:
    ...
    def calculate_position_weights(self) -> None:
        ...
        for token in self._positions:
            weights[token] = sum(1 / number for number in self._positions[token])
        for token in self._positions:
            self._positions_weights[token] = weights[token] / sum(weights.values())
    ...
    def get_position_weights(self) -> dict[int, float]:
        ...
        -------
            dict[int, float]
                position weights for all vertices in the graph
        """
        return self._positions_weights
...
class VanillaTextRank:
    ...
    def get_top_keywords(self, n_keywords: int) -> tuple[int, ...]:
        ...
        """
        sorted_list = sorted(self._scores, key=lambda vertex: self._scores[vertex], reverse=True)
