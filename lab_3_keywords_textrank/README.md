...
Реализуйте метод, возвращающий определенное количество самых важных токенов с точки зрения `word scores`. Метод принимает один аргумент, обозначающий требуемое количество ключевых слов. Метод возвращает кортеж из наиболее важных токенов.
...
Теперь, когда у нас есть четыре класса, решающие одну и ту же задачу и имеющие один и тот же интерфейс, мы можем приступить непосредственно к их сравнению. Для этого реализуем класс `KeywordExtractionBenchmark`.

#### Шаг 12.1 Конструктор класса
...
Обратите внимание, что порядок тем в `themes` совпадает с порядком текстов в `benchmark_materials`: так, текст и ключевые слова с индексом 0 относятся к теме `culture`, а текст и ключеввые слова с индексом 8 относятся к теме `tech`.
...
#### Шаг 12.2 Подсчет метрики `Recall`
...
Предсказанные ключевые слова содержат следующие настоящие ключевые слова: `'milan', 'dolce', 'gabbana', 'design'`. Их четыре, поэтому значение $True Positive$ равно `4`. 

Следующие настоящие ключевые слова не были обнаружены алгоритмом: `'judge', 'inquiry', 'madonna', 'evidence', 'couple'`, их 5, поэтому значение $False Negative$ равно `5`. 

Тогда $Recall$, согласно формуле, равен $\frac{TP}{TP + FN} = \frac{4}{4 + 5} = 0.(4)$

Реализуйте метод, вычисляющий значение полноты. Метод принимает предсказанные ключевые слова и правильные ключевые слова. Метод возвращает значение полноты.
...
* для каждой из тем считывается новостной транскрипт и соответствующие ему правильные ключевые слова
  ...
  * создаются объекты `VanillaTextRank`, `PositionBiasedTextRank`, `TFIDFAdapter`, `RAKEAdapter`
