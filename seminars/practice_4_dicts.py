...
# assert extract_older_people({'Andrej': 22, 'Alexander': 28, 'Irine': 20}, 20) == ['Andrej', 'Alexander']
...
# Function calls with expected result
# assert remove_duplicates({'Marat': 10000, 'Yaroslav': 15000, 'Sasha': 10000}) == {'Yaroslav': 15000}
...
def bake_cakes(recipe: dict[str, int], ingredients: dict[str, int]) -> int:
    ...
    Write a function bake_cakes(), which takes the recipe (dict) and
    the available ingredients (also dict) and returns the maximum number of cakes Pete can bake (integer).
