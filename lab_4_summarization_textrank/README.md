...
Нумерация позиций в тексте начинается с нуля.
...
методы присваивания (`set_text`, `set_preprocessed`, `set_encoded`) и обращения (`get_text`, `get_preprocessed`, `get_encoded`, `get_position`) к ним.

Методы `set_text`, `set_preprocessed` и `set_encoded` принимают на вход соответствующее значение и 
...
Обратите внимание, что `set_encoded` и `set_preprocessed` могут принимать пустые аргументы. 
...
Обратите внимание, что последовательность стоп-слов _может_ быть пустой. 
...
Также заведите атрибут, в котором будет лежать значение последнего присвоенного числового идентификатора. 
Он пригодится вам при дальнейшей работе.
...
Обратите внимание, что любой из аргументов может быть пустым. В случае, если хотя бы одна из входных последовательностей пустая, 
значение меры Жаккара равно 0.
...
Обратите внимание, что текст может содержать одинаковые предложения. Метод не должен передавать `add_edge` одинаковые предложения. 
Предложения считаются одинаковыми, если у них совпадает возвращаемое значение `get_encoded`. 
